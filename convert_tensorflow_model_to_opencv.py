                ...
                for node in graph_def.node:
                    for j in range(len(node.input)):
                        if name == node.input[j]:
                            node.input[j] = inp
                            print(graph_def.node[i]) # for debugging
...
def transform_graph(input_graph, output_graph=None):
    ...
    new_graph_def = TransformGraph(graph_def, ['Webcam/fifo_queue_DequeueMany:0','Webcam/fifo_queue_DequeueMany:1','Webcam/fifo_queue_DequeueMany:2'], 
                      
            ['hourglass/hg_2/after/hmap/conv/BiasAdd:0','upscale/Mean', 'radius/out/fc/BiasAdd:0'], 

            ['strip_unused_nodes(type=float, shape="1,36,60,1")', 'remove_nodes(op=Identity, op=CheckNumerics, op=Switch)', 
...
def convert_to_constant(input_graph, output_graph=None):
    ...
    use_batch_statistics = tf.constant(False, dtype=tf.bool, shape=[], name='use_batch_statistics') # placeholder in GazeML Model
    learning_rate_multiplier_placeholder = tf.constant(False, dtype=tf.bool, shape=[], name='learning_rate_multiplier_placeholder') # placeholder in GazemML
...
def remove_dropout(input_graph, output_graph=None):
    ...
    tf.io.write_graph(new_graph_def, "", output_graph_pbtxt, as_text=True)
...
if __name__ == '__main__':
    ...
    output_graph_pbtxt = "./frozen_inference_graph_dnn.pbtxt"
